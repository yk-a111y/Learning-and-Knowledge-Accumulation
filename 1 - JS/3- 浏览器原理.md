...
> _promise对象是子协程生成的Promise对象

_promise.then((value) => { ... })中的回调激活后，父协程将控制权交给foo函数的协程，同时将 await 后面执行的结果作为value

foo协程将100赋值给a，然后foo继续执行下面的代码，打印 100 和 2

<font color="#d83931">async 底层就是将函数变为生成器函数，并创建该函数的协程</font>
<font color="#d83931">await 底层就是new一个Promise对象，父协程调用该Promise对象的then方法激活回调，然后函数协程开始执行</font>
