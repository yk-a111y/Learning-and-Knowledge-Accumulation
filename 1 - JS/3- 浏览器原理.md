...
#### 浏览器事件循环
js通过将函数执行上下文压入执行栈执行代码。执行同步代码时，遇到异步事件，浏览器不会等待其返回结果，而是将异步事件挂起，继续向下执行其他函数。同步任务执行结束后，将异步API的回调放入一个任务队列中。任务队列有两种: **微任务队列**和**宏任务队列**。执行栈内的事件执行完毕后，check消息队列，每个消息队列中的任务都是宏任务，执行宏任务后，不急着执行下一个宏任务，而是先将该宏任务的微任务队列清空。
